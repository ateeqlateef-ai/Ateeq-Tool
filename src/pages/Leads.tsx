    ...
    <div className="space-y-8 pb-20">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-white/40 mt-2 font-mono text-xs uppercase tracking-widest">CENTRALIZED PROSPECT MANAGEMENT DATABASE</p>
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-white/5 text-[10px] uppercase tracking-[0.2em] font-bold text-white/30 font-mono">
                <th className="px-8 py-5">Company & Website</th>
              ...
              {loading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    <td colSpan={5} className="px-8 py-10 bg-white/5 opacity-20 border-b border-white/5" />
                  </tr>
                ))
              ) : filteredLeads.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-8 py-20 text-center text-white/20 italic">No leads found matching your criteria.</td>
                </tr>
                ...
                <tr key={lead.id} className="hover:bg-white/5 transition-colors group">
                  <td className="px-8 py-6">
                    <div>
                      ...
                      <a href={lead.website} target="_blank" rel="noreferrer" className="text-white/40 text-[10px] flex items-center gap-1 mt-1 hover:text-white transition-all">
                  ...
                  <td className="px-8 py-6">
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-white/40 font-mono">{lead.phone}</p>
                    </div>
                  </td>
                  <td className="px-8 py-6">
